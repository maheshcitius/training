...
function getAllUsers() {
    ...
    function success(payload) { return { type: actionTypes.GETALL_SUCCESS, payload } }
    function failure(payload) { return { type: actionTypes.GETALL_FAILURE, payload } }
}
...
function updateUser(id,updatePayload) {
    ...
    function success(payload) { return { type: actionTypes.UPDATE_USER_SUCCESS, payload } }
    function failure(payload) { return { type: actionTypes.UPDATE_USER_FAILURE, payload } }
}
...
function deleteUser(id) {
    ...
    return dispatch => {
       ...
       userInformation.deleteUser(id)
            .then(
                response => {
                    payload.message = 'User deleted'
                    payload.userId = id
                    dispatch(success(payload))
                    dispatch(toggleSnackbarOpen({message:'Deleted Successfully',type:'success'}));
                }
            )
            .catch(error=>{
                payload.message = error.response.data;
                payload.userId  = id;
                dispatch(failure(payload))
                dispatch(toggleSnackbarOpen({message:'Failed to Delete',type:'warning'}));
            })
    };
