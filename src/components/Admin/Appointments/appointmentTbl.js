...
const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - appointmentss.length) : 0;

const appointments = appointmentss.length>0  ? applySortFilter(appointmentss, getComparator(order, orderBy), filterName) : [];
                ...
                <TableBody>
                  {appointments
                    ...
                    .map((row) => {
                      ...
                      const { id, title, physicianName, visitType, scheduleDate, scheduleTime , status } = row;
